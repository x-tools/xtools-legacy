...
class gd_gradient_fill {
        ...
        for ( $i = 0; $i < $line_numbers; $i=$i+1+$this->step ) {
            ...
            switch($direction) {
                case 'vertical':
                    ...
                    break;
                case 'random':
                	for ( $jij = 0; $jij < $line_width; $jij++ ) {
                		if( $r + 3 > 255 ) $r -= 3;
				    	if( $g + 3 > 255 ) $g -= 3;
				    	if( $b + 3 > 255 ) $b -= 3;
				    	
                		list( $r123, $g123, $b123 ) = $this->genRandomVals( $r, $g, $b );
	                	//if ( "$old_r,$old_g,$old_b" != "$r1,$g1,$b1") 
	                		$fill = imagecolorallocate( $im, $r123, $g123, $b123 );
	                		
	                	//imagesetpixel($im, $i, $jij, $fill);
	                	imagefilledrectangle($im, $jij, $i, $jij, $i+$this->step, $fill);
                	}
    ...
    function genRandomVals( $r, $g, $b ) {
    	...
    	return array( $r + $rand, $g + $rand, $b + $rand );
    	
    }
    
    function addRand( $rand, $r ) {
    	return $rand + $r;
    }
}
